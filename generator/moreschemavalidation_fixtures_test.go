...
func initFixtureGuardFormats() {
	...
	flattenRun.AddExpectations("object.go", []string{
		...
		`	if typeutils.IsZero(m.P1Nullable) {`,
...
func initFixture1479Part() {
	...
	*/
	...
	flattenRun.AddExpectations("container_create_config_all_of1.go", []string{
		...
		`	if typeutils.IsZero(m.HostConfig) {`,
		`	if m.HostConfig != nil {`,
		...
		`	if typeutils.IsZero(m.NetworkingConfig) {`,
	...
	flattenRun.AddExpectations("container_config.go", []string{
		...
		`	if typeutils.IsZero(m.ExposedPorts) {`,
		...
		`	if typeutils.IsZero(m.Hostname) {`,
		...
		`	if typeutils.IsZero(m.RichMode) {`,
	...
	flattenRun.AddExpectations("host_config_all_of0.go", []string{
		...
		`	if typeutils.IsZero(m.ConsoleSize) {`,
		...
		`	if typeutils.IsZero(m.Isolation) {`,
		...
		`	if typeutils.IsZero(m.LogConfig) {`,
		`	if m.LogConfig != nil {`,
		...
		`	if typeutils.IsZero(m.OomScoreAdj) {`,
		...
		`	if typeutils.IsZero(m.RestartPolicy) {`,
	...
	expandRun.AddExpectations("host_config.go", []string{
		...
		`	if typeutils.IsZero(m.BlkioWeight) {`,
	...
	expandRun.AddExpectations("container_create_config.go", []string{
		...
		`	if typeutils.IsZero(m.EndpointsConfig) {`,
		`	for k := range m.EndpointsConfig {`,
		`		if typeutils.IsZero(m.EndpointsConfig[k]) {`,
		...
		`	if typeutils.IsZero(m.IPAMConfig) {`,
		`	if m.IPAMConfig != nil {`,
...
func initFixtureSimpleAllOf() {
	...
	expandRun.AddExpectations("break_nested_object.go", []string{
		...
		`	_parts := make([][]byte, 0, 2`,
		`	aO0, err := jsonutils.WriteJSON(m.BreakNestedObjectAllOf0`,
		`	if err != nil {`,
		`		return nil, err`,
		...
		`	_parts := make([][]byte, 0, 2`,
		`	aO0, err := jsonutils.WriteJSON(m.BreakNestedObjectAllOf0AllOf0`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	var dataAO1 struct {`,
	...
	expandRun.AddExpectations("deep_nested_object.go", []string{
		...
		`	_parts := make([][]byte, 0, 2`,
		`	aO0, err := jsonutils.WriteJSON(m.DeepNestedObjectAllOf0`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	aO1, err := jsonutils.WriteJSON(m.DeepNestedObjectAllOf1`,
		`	if err != nil {`,
		`		return nil, err`,
		...
		`	_parts := make([][]byte, 0, 2`,
		`	aO0, err := jsonutils.WriteJSON(m.DeepNestedObjectAllOf0AllOf0`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	var dataAO1 struct {`,
	...
	expandRun.AddExpectations("simple_nested_object.go", []string{
		...
		`	_parts := make([][]byte, 0, 2`,
		`	aO0, err := jsonutils.WriteJSON(m.SimpleNestedObjectAllOf0`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	var dataAO1 struct {`,
...
func initFixtureItching() {
	...
	*/
	...
	flattenRun.AddExpectations("empty_object_with_additional_slice_additional_properties_items.go", []string{
		...
		`	if typeutils.IsZero(m.DummyProp1) {`,
	...
	flattenRun.AddExpectations("object_with_empty_object.go", []string{
		...
		`	if typeutils.IsZero(m.EmptyObj) {`,
		...
		`	if typeutils.IsZero(m.NonEmptyObj) {`,
	...
	expandRun.AddExpectations("object_with_empty_object.go", []string{
		...
		`		if typeutils.IsZero(m.EmptyObj[k]) {`,
...
func initFixtureAdditionalProps() {
	...
	*/
	...
	flattenRun.AddExpectations("additional_array_of_refed_thing.go", []string{
		...
		`	if typeutils.IsZero(m.ThisOneNotRequired) {`,
	...
	flattenRun.AddExpectations("additional_object_with_nullable_thing.go", []string{
		...
		`		if typeutils.IsZero(m.AdditionalObjectWithNullableThing[k]) {`,
	...
	flattenRun.AddExpectations("additional_slice_of_aliased_nullable_primitives.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`			if typeutils.IsZero(m.AdditionalSliceOfAliasedNullablePrimitives[k][i]) {`,
		// nullable required:
	...
	flattenRun.AddExpectations("additional_slice_of_slice.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`				if typeutils.IsZero(m.AdditionalSliceOfSlice[k][i][ii]) {`,
		// nullable not required:
	...
	flattenRun.AddExpectations("additional_things_nested_additional_properties.go", []string{
		...
		`	if typeutils.IsZero(m.PrinterCountry) {`,
		...
		`	if typeutils.IsZero(m.PrinterDate) {`,
	...
	flattenRun.AddExpectations("additional_things_nested_additional_properties_additional_properties.go", []string{
		...
		`	if typeutils.IsZero(m.AverageDelay) {`,
	...
	flattenRun.AddExpectations("additional_transitive_refed_thing.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`			if typeutils.IsZero(m.AdditionalTransitiveRefedThing[k][i]) {`,
		// nullable required:
	...
	expandRun.AddExpectations("additional_transitive_refed_thing.go", []string{
		...
		`	if typeutils.IsZero(m.ThisOneNotRequiredEither) {`,
	...
	flattenRun.AddExpectations("additional_slice_of_objects.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`			if typeutils.IsZero(m.AdditionalSliceOfObjects[k][i]) {`,
		// nullable required:
	...
	flattenRun.AddExpectations("additional_array_of_refed_object.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`			if typeutils.IsZero(m.AdditionalArrayOfRefedObject[k][i]) {`,
		// nullable required:
	...
	flattenRun.AddExpectations("additional_date_with_nullable_thing.go", []string{
		...
		`		if typeutils.IsZero(m.AdditionalDateWithNullableThing[k]) {`,
		...
		`	if typeutils.IsZero(m.NullableDate) {`,
...
func initFixtureTuple() {
	...
	expandRun.AddExpectations("classics.go", []string{
		...
		`	if typeutils.IsZero(m.Character) {`,
...
func initFixture1042() {
	...
	*/
	...
	flattenRun.AddExpectations("b.go", []string{
		...
		`	_parts := make([][]byte, 0, 2)`,
		`	aO0, err := jsonutils.WriteJSON(m.A`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	aO1, err := jsonutils.WriteJSON(m.BAllOf1`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO1`,
		`	return jsonutils.ConcatJSON(_parts...), nil`,
...
func initFixture1042V2() {
	...
	*/
	...
	flattenRun.AddExpectations("extended_error_model.go", []string{
		...
		// slight optimization of allocations
		`	_parts := make([][]byte, 0, 2)`,
		`	aO0, err := jsonutils.WriteJSON(m.ErrorModel`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	aO1, err := jsonutils.WriteJSON(m.ExtendedErrorModelAllOf1`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO1`,
		`	return jsonutils.ConcatJSON(_parts...), nil`,
...
func initFixture979() {
	...
	flattenRun.AddExpectations("cluster.go", []string{
		...
		// slight optimization of allocations
		`	_parts := make([][]byte, 0, 2)`,
		`	aO0, err := jsonutils.WriteJSON(m.NewCluster`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	aO1, err := jsonutils.WriteJSON(m.ClusterAllOf1`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO1`,
		`	return jsonutils.ConcatJSON(_parts...), nil`,
...
func initTodolistSchemavalidation() {
	...
	*/
	...
	flattenRun.AddExpectations("pet.go", []string{
		...
		`	if typeutils.IsZero(m.Category) {`,
		`	if m.Category != nil {`,
	...
	flattenRun.AddExpectations("update_org.go", []string{
		...
		`	if typeutils.IsZero(m.TagExpiration) {`,
...
func initFixtureMoreAddProps() {
	...
	*/
	...
	flattenRun.AddExpectations("trial.go", []string{
		...
		`	if typeutils.IsZero(m.AdditionalProperties) {`,
...
func initFixture1537() {
	...
	*/
	...
	thisRun.AddExpectations("profile_array.go", []string{
		...
		// do we need Required when item is nullable?
		// nullable not required:
		`		if typeutils.IsZero(m.ProfileCfg[i]) {`,
		// nullable required:
	...
	thisRun.AddExpectations("profile.go", []string{
		...
		`	if typeutils.IsZero(m.ProfileCfg) {`,
...
func initFixture1617() {
	...
	thisRun.AddExpectations("artifact_info.go", []string{
		...
		"		Status ArtifactStatus `json:\"Status,omitempty\"`",
		"		Timestamp strfmt.DateTime `json:\"Timestamp,omitempty\"`",
		`	if err := jsonutils.ReadJSON(raw, &propsArtifactInfo); err != nil {`,
		...
		`	_parts := make([][]byte, 0, 1`,
		`	aO0, err := jsonutils.WriteJSON(m.ArtifactDescription`,
		`	if err != nil {`,
		`		return nil, err`,
		`	_parts = append(_parts, aO0`,
		`	var propsArtifactInfo struct {`,
		...
		`	jsonDataPropsArtifactInfo, errArtifactInfo := jsonutils.WriteJSON(propsArtifactInfo`,
		...
		`	if typeutils.IsZero(m.Timestamp) {`,
