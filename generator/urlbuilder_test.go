	...
	if assert.NoError(err) {
		...
		if assert.NoError(err) {
			...
			if assert.NoError(err) {
				...
				if assert.NoError(err) {
					res := string(ff)
					assertInCode(t, "var path = \"/singleValuePath/{siString}/{siInt}/{siInt32}/{siInt64}/{siFloat}/{siFloat32}/{siFloat64}/{siBool}\"", res)
					assertInCode(t, "siBool := swag.FormatBool(o.SiBool)", res)
					assertInCode(t, `if siBool != ""`, res)
					assertInCode(t, `path = strings.Replace(path, "{siBool}", siBool, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siFloat}", siFloat, -1)`, res)
					assertInCode(t, `siFloat32 := swag.FormatFloat32(o.SiFloat32)`, res)
					assertInCode(t, `if siFloat32 != "" `, res)
					assertInCode(t, `path = strings.Replace(path, "{siFloat32}", siFloat32, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siFloat64}", siFloat64, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siInt}", siInt, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siInt32}", siInt32, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siInt64}", siInt64, -1)`, res)
					...
					assertInCode(t, `path = strings.Replace(path, "{siString}", siString, -1)`, res)
					assertInCode(t, `return nil, errors.New("SiString is required on SimplePathParamsURL")`, res)
					assertInCode(t, `result.Path = path`, res)
	...
	if assert.NoError(err) {
		...
		if assert.NoError(err) {
			...
			if assert.NoError(err) {
				...
				if assert.NoError(err) {
					res := string(ff)
					assertInCode(t, "var path = \"/singleValueQuery/{id}\"", res)
					assertInCode(t, `id := swag.FormatInt32(o.ID)`, res)
					assertInCode(t, `if id != ""`, res)
					assertInCode(t, `path = strings.Replace(path, "{id}", id, -1)`, res)
		...
		if assert.NoError(err) {
			...
			if assert.NoError(err) {
				...
				if assert.NoError(err) {
					...
					assertInCode(t, "var path = \"/arrayValueQuery/{id}\"", res)
					assertInCode(t, `id := swag.FormatInt32(o.ID)`, res)
					assertInCode(t, `if id != ""`, res)
					assertInCode(t, `path = strings.Replace(path, "{id}", id, -1)`, res)
