...
func (o *GetOrdersForItemURL) Build() (*url.URL, error) {
	...
	var _path = "/orders/{itemID}"

	itemID := o.ItemID
	if itemID != "" {
		_path = strings.ReplaceAll(_path, "{itemID}", itemID)
	} else {
		return nil, errors.New("itemId is required on GetOrdersForItemURL")
	}
...
func (o *GetOrdersForItemURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetOrdersForItemURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetOrdersForItemURL")
	}
