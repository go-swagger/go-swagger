...
func WithContentTypeApplicationIoGoswaggerExamplesTodoListV1JSON(r *runtime.ClientOperation) {
	r.ConsumesMediaTypes = []string{"application/io.goswagger.examples.todo-list.v1+json"}
}
...
func WithAcceptApplicationIoGoswaggerExamplesTodoListV1JSON(r *runtime.ClientOperation) {
	r.ProducesMediaTypes = []string{"application/io.goswagger.examples.todo-list.v1+json"}
}
...
func (a *Client) PutTest2766(params *PutTest2766Params, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*PutTest2766OK, error) {
	...
	success, ok := result.(*PutTest2766OK)
	if ok {
		return success, nil
	}
	...
	msg := fmt.Sprintf("unexpected success response for PutTest2766: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
