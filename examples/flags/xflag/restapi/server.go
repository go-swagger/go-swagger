...
func (s *sliceValue) Get() any { return []string(*s) }
...
func init() {
	...
	flag.StringVar(&specFile, "spec", "", "the swagger specification to serve")
	flag.Var(newSliceValue(defaultSchemes, &enabledListeners), "scheme", "the listeners to enable, this can be repeated and defaults to the schemes in the swagger spec")
...
// NewServer creates a new api simple to do list API server but does not configure it
func NewServer(api *operations.SimpleToDoListAPIAPI) *Server {
	s := new(Server)
...
// Server for the simple to do list API API
type Server struct {
	EnabledListeners []string
	CleanupTimeout   time.Duration
	GracefulTimeout  time.Duration
	...
	Spec         string
	api          *operations.SimpleToDoListAPIAPI
...
func (s *Server) SetAPI(api *operations.SimpleToDoListAPIAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving simple to do list API at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := domainSocket.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving simple to do list API at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving simple to do list API at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving simple to do list API at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving simple to do list API at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpsServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving simple to do list API at https://%s", l.Addr())
