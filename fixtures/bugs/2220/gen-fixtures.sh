...
testcases="${testcases} fixture-2220.yaml"
for opts in  "" "--strict-additional-properties" ; do
for testcase in ${testcases} ; do
    ...
    if [[ -z ${opts} ]]; then
        ...
        target=./gen-${testcase%.*}-strict
    fi
    serverName="codegensrv"
    rm -rf ${target}
    mkdir ${target}
    echo "Serrer generation for ${spec} with opts=${opts}"
