...
| The shell prints only `zsh: killed` or `Killed` after generation starts | The OS terminated the process; the two most common causes are (1) CGO is enabled in a source build while macOS SIP blocks certain dynamic libraries, and (2) the process was killed by the OOM killer or a resource limit | See the section below |

### `zsh: killed` or `Killed` during generation

The official release binaries are built with `CGO_ENABLED=0`.
When you build from source with the default toolchain, CGO is enabled, and macOS System Integrity Protection (SIP) may terminate the resulting binary.
...
When reporting this problem, include the exact `swagger generate` command, `swagger version`, operating system, whether the binary was installed from a release or built from source, and any OOM or SIP evidence from the system logs.
